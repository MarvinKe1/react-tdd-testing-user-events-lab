...
test("the page shows information the user types into the name and email address form fields", async () => {
  ...
  await userEvent.type(nameInput, "Marvin");
  await userEvent.type(emailInput, "marvin@example.com");

  expect(nameInput).toHaveValue("Marvin");
  expect(emailInput).toHaveValue("marvin@example.com");
});

test("checked status of checkboxes changes when user clicks them", async () => {
  render(<App />);
  const [codingCheckbox, designCheckbox, writingCheckbox] = screen.getAllByRole("checkbox");
  ...
  await userEvent.click(writingCheckbox);
  expect(writingCheckbox).toBeChecked();
});
...
test("a message is displayed when the user clicks the Submit button", async () => {
  ...
  expect(screen.getByText(/thank you, marvin/i)).toBeInTheDocument();
  expect(screen.getByText(/signed up successfully with marvin@example.com/i)).toBeInTheDocument();
});
